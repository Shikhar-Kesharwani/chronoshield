...
def _push(message: Dict[str, Any]):
    ...
    if REDIS_MODE == "real":
        try:
            ...
            log.warning(
                f"Redis push failed ({e}), falling back to mock queue."
            )
...
def run_generator(stop_event: threading.Event = None):
    ...
    tick = 0
    log.info(
        f"Generator started — metrics=[cpu, memory, latency], rate={GENERATOR_RATE}/s, mode={REDIS_MODE}"
    )
        ...
        for metric, p in metrics.items():
            ...
            if manual_spike:
                if metric == "cpu":
                    ...
                    spike = random.uniform(300, 800)
                value += spike
                injected = True
                log.info(f"Manual anomaly injected on {metric}: {value:.2f}")
            elif random.random() < GENERATOR_ANOMALY_PROB:
                if metric == "cpu":
                    spike = random.choice([-1, 1]) * random.uniform(20, 50)
                elif metric == "memory":
                    spike = random.choice([-1, 1]) * random.uniform(10, 25)
                else:
                    spike = random.choice([-1, 1]) * random.uniform(200, 600)
                value += spike
                injected = True
